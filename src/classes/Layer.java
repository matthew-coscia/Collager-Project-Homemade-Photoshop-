...
public class Layer {
   ...
   * in the making of a layer.
   * @param pixels represents the pixels that make up a layer.
   * @param name represents the name of a given layer.
   * @param state represents the current state of the layer.
   * @param controller represents the user interaction of the program.
   * @param model is a place where all processes for the collager
   *              application go.
  ...
  public Layer(ArrayList<ArrayList<PixelRGB>> pixels, String name, CollagerState state,
               CollagerController controller, CollagerModel model, TextView view) {
    this.pixels = pixels;
   ...
   * creates a new layer.
   * @param pixels represents the pixels that make up a given image or layer.
   * @param name represents the name of the layer.
   * @param filterOnCurrentLayer represents the filter that is currently
   *                             applied to the layer.
   * @param state represents the current state of the layer.
   * @param model represents where all processes for the application
   *              are done.
   * @param controller represents the user interaction of the program.
   * @param view represents how the user receives output from the program.
   */
  public Layer(ArrayList<ArrayList<PixelRGB>> pixels, String name, String filterOnCurrentLayer,
               CollagerState state, CollagerModel model,
  ...
  /**
   * A method that gives the pixels to save an image. Used to call this
   * field in other classes.
   * @return the pixels that are saved.
   */
  ...
  public void blendMultiply(int layerPos) {
    for (int i = 0; i < this.saveImagePixels.size(); i++) {
      for (int k = 0; k < this.saveImagePixels.get(i).size(); k++) {
        ...
        this.saveImagePixels.get(i).set(k, new PixelRGB(
                pixelHSL, this.state, this.model, this.controller));
      }
    }
  ...
  public void blendScreen(int layerPos) {
    for (int i = 0; i < this.saveImagePixels.size(); i++) {
      for (int j = 0; j < this.saveImagePixels.get(i).size(); j++) {
        ...
        this.saveImagePixels.get(i).set(j, new PixelRGB(
                pixelHSL, this.state, this.model, this.controller));
      }
    }
  }
